...
namespace Dafy.OnlineTran.Entity.Models
{
    ...
    public partial class Role : Entity<Role>
    {
		...
		/// <summary>该角色表所拥有的用户表集合</summary>
		[XmlIgnore]
		public EntityList<User> Users
		{
			get
			{
				if (_Users == null && roleId > 0 && !Dirtys.ContainsKey("Users"))
                {
					_Users = User.FindAllByroleId(roleId);
					Dirtys["Users"] = true;
				}
				return _Users;
			}
        ...
        public static Role FindByroleId(Int64 roleid)
        {
            if (Meta.Count >= 1000)
